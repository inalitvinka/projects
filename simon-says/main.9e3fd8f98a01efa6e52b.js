...
"use strict";
module.exports = __webpack_require__.p + "assets/fef2c4c93609d2b93c18.png";
...
var ___HTML_LOADER_IMPORT_0___ = new URL(/* asset import */ __webpack_require__(7621), __webpack_require__.b);
// Module
var ___HTML_LOADER_REPLACEMENT_0___ = getUrl_default()(___HTML_LOADER_IMPORT_0___);
var code = "<!DOCTYPE html> <html lang=\"en\"> <head> <meta charset=\"UTF-8\"> <meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"> <link rel=\"icon\" href=\"" + ___HTML_LOADER_REPLACEMENT_0___ + "\" type=\"image/png\"> <title>Simon Says</title> </head> <body> </body> </html> ";
// Exports
/* harmony default export */ var src = ((/* unused pure expression or super */ null && (code)));
;// CONCATENATED MODULE: ./src/scripts/util/createComponent.js
...
/* harmony default export */ var createComponent = (createElement);
;// CONCATENATED MODULE: ./src/scripts/util/globalVars.js
const vars = {
  currentRound: 0,
  roundsNumber: 5,
  sequenceLength: 2,
...
/* harmony default export */ var globalVars = (vars);
...
const textContent = {
  TITLE: 'simon says:',
  COUNTER: 'round:',
  CURRENT: `${globalVars.currentRound}`,
  ROUNDS: ` / ${globalVars.roundsNumber}`,
  BTN_ATR: 'data-btn',
  START_BTN: 'start',
  START_ATR: 'start',
  NEXT_BTN: 'next round',
...
;// CONCATENATED MODULE: ./src/scripts/util/disableKeyboard.js
...
const setDisabledKeyboard = components => {
  ...
  globalVars.isDisabled = true;
};
/* harmony default export */ var disableKeyboard = (setDisabledKeyboard);
;// CONCATENATED MODULE: ./src/scripts/util/initGame.js

const initGame = components => {
  disableKeyboard(components);
...
/* harmony default export */ var util_initGame = (initGame);
;// CONCATENATED MODULE: ./src/scripts/util/showUserSequence.js

const showUserSequence = components => {
  components.output.textContent = globalVars.userInputArr.join('');
};
/* harmony default export */ var util_showUserSequence = (showUserSequence);
;// CONCATENATED MODULE: ./src/scripts/plugins/modal.js

const modal_cssClasses = {
  OPEN: 'open',
  LOCK: 'lock',
  HIDE: 'hiding'
};
const modalWindow = () => {
  ...
  const modal = {
    open() {
      !isClosing && globalVars.modal.winwow.classList.add(modal_cssClasses.OPEN);
      !isClosing && document.body.classList.add(modal_cssClasses.LOCK);
    },
    close() {
      isClosing = true;
      globalVars.modal.winwow.classList.remove(modal_cssClasses.OPEN);
      globalVars.modal.winwow.classList.add(modal_cssClasses.HIDE);
      document.body.classList.remove(modal_cssClasses.LOCK);
      globalVars.modal.winwow.classList.add(modal_cssClasses.HIDE);
      setTimeout(() => {
        globalVars.modal.winwow.classList.remove(modal_cssClasses.HIDE);
        isClosing = false;
      }, TRANSITION_SPEED);
    },
    setContent(result) {
      let content = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'Congrats! You\'ve won!';
      globalVars.modal.modalContent.textContent = content;
      globalVars.modal.modalResult.textContent = result;
    }
  };
...
;// CONCATENATED MODULE: ./src/scripts/util/checkUserSequence.js



const checkUserSequence = components => {
  for (let i = 0; i < globalVars.userInputArr.length; i++) {
    if (globalVars.userInputArr[i] !== globalVars.reviewSequence[i] && globalVars.attempts) {
      globalVars.attempts -= 1;
      components.output.textContent = 'Wrong answer!';
      disableKeyboard(components);
    }
    if (globalVars.userInputArr[i] !== globalVars.reviewSequence[i] && !globalVars.attempts) {
      plugins_modal.setContent(`Simon says: ${globalVars.reviewSequence.join('')}`, 'You\'ve lost!');
      plugins_modal.open();
      disableKeyboard(components);
    }
    if (globalVars.userInputArr.length === globalVars.reviewSequence.length && globalVars.userInputArr.join('') === globalVars.reviewSequence.join('') && globalVars.currentRound !== globalVars.roundsNumber) {
      components.output.textContent = 'Done!';
      components.nextBtn.classList.remove('hidden');
      components.repeatBtn.classList.add('hidden');
      disableKeyboard(components);
    }
    if (globalVars.currentRound === globalVars.roundsNumber && globalVars.userInputArr.length === globalVars.reviewSequence.length && globalVars.userInputArr.join('') === globalVars.reviewSequence.join('')) {
      plugins_modal.setContent(`Simon says: ${globalVars.reviewSequence.join('')}`);
      plugins_modal.open();
      disableKeyboard(components);
...
/* harmony default export */ var util_checkUserSequence = (checkUserSequence);
;// CONCATENATED MODULE: ./src/scripts/app/renderKeyboard.js




const renderKeyboard_cssClasses = {
  ITEM: 'keyboard-item'
};
const renderKeyboard = (obj, container, components) => {
  for (let i = 0; i < Object.keys(obj).length; i++) {
    const key = createComponent({
      tag: 'button',
      classes: [renderKeyboard_cssClasses.ITEM],
      textContent: obj[Object.keys(obj)[i]]
    });
    key.addEventListener('click', event => {
      globalVars.userInputArr.push(event.target.textContent);
      util_showUserSequence(components);
      util_checkUserSequence(components);
    });
    container.append(key);
  }
};
/* harmony default export */ var app_renderKeyboard = (renderKeyboard);
;// CONCATENATED MODULE: ./src/scripts/app/createNavButtons.js


const createNavButtons_cssClasses = {
  BUTTON: 'nav-button'
};
const createNavButtons_textContent = {
  BTN_ATR: 'data-btn'
};
const levels = [{
  name: 'easy'
...
const createNavButtons = (container, nums, letters) => {
  levels.forEach((item, index) => {
    const btn = createComponent({
      tag: 'button',
      classes: [createNavButtons_cssClasses.BUTTON],
      textContent: item.name,
      attribute: [createNavButtons_textContent.BTN_ATR, item.name]
    });
    btns.push(btn);
    if (!index) {
      btn.classList.add('nav-selected');
...
/* harmony default export */ var app_createNavButtons = (createNavButtons);
;// CONCATENATED MODULE: ./src/scripts/util/setNotDisabledKeyboard.js
...
const setNotDisabledKeyboard = components => {
  ...
  globalVars.isDisabled = false;
};
/* harmony default export */ var util_setNotDisabledKeyboard = (setNotDisabledKeyboard);
;// CONCATENATED MODULE: ./src/scripts/util/showSequence.js
...
/* harmony default export */ var util_showSequence = (showSequence);
;// CONCATENATED MODULE: ./src/scripts/util/generateRandomSequence.js

const generateRandomSequence = components => {
  let keys;
  let randomIndex;
  globalVars.sequence.length = 0;
  globalVars.reviewSequence.length = 0;
  if (globalVars.level === 'easy') {
    keys = components.keyboardNums.children;
  }
  if (globalVars.level === 'medium') {
    keys = components.keyboardLetters.children;
  }
  if (globalVars.level === 'hard') {
    keys = [...components.keyboardNums.children, ...components.keyboardLetters.children];
  }
  do {
    randomIndex = Math.floor(Math.random() * keys.length);
    globalVars.sequence.push(keys[randomIndex]);
    globalVars.reviewSequence.push(keys[randomIndex].textContent);
  } while (globalVars.reviewSequence.length < globalVars.sequenceLength);
  console.log(`Simon says: ${globalVars.reviewSequence.join(' ')}`);
};
/* harmony default export */ var util_generateRandomSequence = (generateRandomSequence);
;// CONCATENATED MODULE: ./src/scripts/util/handleGameBtns.js




const handleGameBtns = components => {
  const _updateCurrentRound = () => components.currentRound.textContent = globalVars.currentRound;
  const ATTEMPTS = 2;
  ...
  document.addEventListener('click', event => {
    ...
    if (target === 'repeat') {
      ...
      globalVars.userInputArr.length = 0;
      util_showSequence(globalVars.sequence, components);
    }
    if (target === 'start') {
      globalVars.attempts = ATTEMPTS;
      util_generateRandomSequence(components);
    ...
    if (target === 'new game') {
      ...
      globalVars.userInputArr.length = 0;
      globalVars.attempts = ATTEMPTS;
      globalVars.currentRound = 0;
      globalVars.sequenceLength = INIT_SEQUENCE_LENGTH;
      _updateCurrentRound();
...
/* harmony default export */ var util_handleGameBtns = (handleGameBtns);
;// CONCATENATED MODULE: ./src/scripts/util/keys-data.js
const nums = {
  Digit1: '1',
  Digit2: '2',
  Digit3: '3',
...
;// CONCATENATED MODULE: ./src/scripts/util/handleKeyDown.js
...
const handleKeyDown = components => {
  ...
  document.addEventListener('keydown', event => {
    if (globalVars.isDisabled || !event.code.includes('Digit') && !event.code.includes('Key') || globalVars.level === 'easy' && event.code.includes('Key') || globalVars.level === 'medium' && event.code.includes('Digit') || !isPressed) {
      globalVars.isKey = false;
      return;
    }
    isPressed = false;
    globalVars.isKey = true;
    let activeLetter = event.code.slice(-1).toUpperCase();
    globalVars.userInputArr.push(activeLetter);
    globalVars.keyIndex = Object.keys({
      ...nums,
      ...letters
    }).indexOf(event.code);
    elements[globalVars.keyIndex].classList.add('key-active');
    util_showUserSequence(components);
    util_checkUserSequence(components);
    setTimeout(() => isPressed = true, 100);
  });
  document.addEventListener('keyup', event => {
    if (globalVars.isDisabled || !globalVars.keyIndex) return;
    elements[globalVars.keyIndex].classList.remove('key-active');
  });
  [...components.keyboardNums.children, ...components.keyboardLetters.children].forEach(item => {
    if (item.classList.contains('key-active')) {
      item.classList.remove('key-active');
...
/* harmony default export */ var util_handleKeyDown = (handleKeyDown);
...
const createApp = () => {
  const components = app_createPageLayout();
  app_createNavButtons(components.nav, components.keyboardNums, components.keyboardLetters);
  app_renderKeyboard(nums, components.keyboardNums, components);
  app_renderKeyboard(letters, components.keyboardLetters, components);
  util_handleKeyDown(components);
  util_handleGameBtns(components);
  util_initGame(components);
};
...
/******/ })()
;
//# sourceMappingURL=main.9e3fd8f98a01efa6e52b.js.map